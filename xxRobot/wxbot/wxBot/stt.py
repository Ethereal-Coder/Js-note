...
def speech_to_text(waveData):
    ...
    if sst_static.GLOBAL_isLogin == 0:
        ret = MSPLogin(None,None,'appid = 59de05b6, work_dir = .')
        if ret != 0:
            logging.error("MSPLogin failed, error code: " + ret);
            return
        else:
            sst_static.GLOBAL_isLogin=1
...
if __name__ == '__main__':
    print speech_to_text(getWaveData('1.wav'))

# 语音转换文字
def speech_file_to_text(file):
    ...
    subprocess.call(["ffmpeg", "-y", "-i" , INPUT_FILE, "-acodec", "pcm_s16le", "-ac", "1" , "-ar" , "16000" , 'temp.wav'])

    OUT_FILE = os.path

    return speech_to_text(getWaveData('temp.wav'))